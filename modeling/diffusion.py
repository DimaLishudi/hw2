...
class DiffusionModel(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        timestep = torch.randint(1, self.num_timesteps + 1, (x.shape[0],), device=x.device) # FIX: added device
        eps = torch.randn_like(x)

        x_t = (
            self.sqrt_alphas_cumprod[timestep, None, None, None] * x
            + self.sqrt_one_minus_alpha_prod[timestep, None, None, None] * eps # FIX: Это удар ниже пояса
        )
    ...
    def sample(self, num_samples: int, size, device) -> torch.Tensor:
        ...
        x_i = torch.randn(num_samples, *size, device=device) # FIX: added device

        for i in range(self.num_timesteps, 0, -1):
            z = torch.randn(num_samples, *size, device=device) if i > 1 else 0 # FIX: added device
